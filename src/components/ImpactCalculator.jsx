...
export default function ImpactCalculator({ guidePrice, agencyPrice, currency = '$' }) {
    ...
    <Paper elevation={0} sx={{ p: 2, bgcolor: '#E9D8A6', borderRadius: 3, border: '1px solid rgba(16,42,67,0.08)' }}>
      ...
      <Box sx={{ display: 'flex', gap: 2, mt: 1 }}>
        ...
        <Box sx={{ flex: 1, textAlign: 'center', p: 1.5, bgcolor: '#FFFFFF', borderRadius: 2 }}>
          ...
          <Typography variant="h3" fontWeight={800} sx={{ mt: 0.5 }}>
            {currency}{localRetention.toLocaleString()}
          </Typography>
          <Typography variant="caption" fontWeight={600} color="text.secondary">
      ...
      <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1, textAlign: 'center' }}>
        By booking direct, an extra {currency}{savings.toLocaleString()} stays with the guide's community vs. a standard agency.
      </Typography>
    </Paper>
  );
}
