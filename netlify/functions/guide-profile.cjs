...
const { authenticateGuideOwner, authenticateGuide, json } = require('./auth.cjs');
...
exports.handler = async (event) => {
  ...
  if (method === 'POST' && path[0] === 'submit') {
    const authResult = await authenticateGuide(event);
    if (authResult.statusCode) return authResult;
    const { user: verifiedUser, profile: userProfile, supabase } = authResult;

    const { data: guide } = await supabase.from('guides').select('*').eq('user_id', verifiedUser.id).single();
    ...
    const { data, error } = await supabase.from('guides').update({ status: 'pending', updated_at: new Date().toISOString() }).eq('user_id', verifiedUser.id).select().single();
  ...
  // All other operations require verified guide ownership
  const authResult = await authenticateGuideOwner(event);
  if (authResult.statusCode) return authResult;
  const { user: verifiedUser, supabase, guide } = authResult;
  ...
  if (method === 'PUT') {
    ...
    // Never allow: status, price, price_currency, featured, verification flags,
    // balances, referral, ownership fields.
    ...
    const { data, error } = await supabase.from('guides').update(updates).eq('id', guide.id).select().single();
    if (error) return { statusCode: 500, headers, body: JSON.stringify({ error: error.message }) };
    return { statusCode: 200, headers, body: JSON.stringify(data) };
  }
  ...
  if (method === 'POST' && path[0] === 'routes') {
    ...
    const { data, error } = await supabase.from('guides').update({ routes, updated_at: new Date().toISOString() }).eq('user_id', verifiedUser.id).select().single();
    if (error) return { statusCode: 500, headers, body: JSON.stringify({ error: error.message }) };
    return { statusCode: 200, headers, body: JSON.stringify(data) };
  }
